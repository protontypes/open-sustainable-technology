...
_"Choose a single subsection from OpenSustain.tech and find missing open-source projects that belong there. Use the OpenSustain.tech scope and contribution guidelines to judge relevance. Before proposing any project, check whether it is already listed anywhere on OpenSustain.tech. Only include projects that are not already listed. Return results in a table with these columns: 1. Subsection 2. Project 3. Namespace 4. Repository URL Constraints: 1. Only one project per namespace/organization 2. No duplicates of projects already listed on OpenSustain.tech 3. Only propose projects with a strong, clear match to the chosen subsection 4 Prefer active open-source repositories with meaningful documentation and real sustainability relevance."_
