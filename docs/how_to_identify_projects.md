...
*   **Using LLM prompts to discover missing projects:** Various prompts can help you to identify any missing projects on OpenSustain.tech. Although most LLMs are familiar with some of the projects listed on OpenSustain.tech, it is helpful to provide details of the existing projects. Here is an example of a prompt that works well with most LLMs:
```
Choose one subsection from OpenSustain.tech completely at random and identify any missing open-source projects that should be included. Use the scope and contribution guidelines on OpenSustain.tech to judge relevance. Before proposing a project, check whether it is already listed on OpenSustain.tech. Only include projects that are not already listed. Return results in a table with the following columns: 1. Subsection 2. Project 3. Namespace 4. Repository URL Constraints: 1. Only one project per namespace/organisation 2. There must be no duplicates of projects already listed on OpenSustain.tech. 3. Propose projects that have a strong and clear match with the chosen subsection. Prefer active open-source repositories with meaningful documentation and real sustainability relevance.
```
